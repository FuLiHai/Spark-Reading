 ...
 * speculative tasks, etc.
  *
  * 在不同的集群上通过SchedulerBackend上调度任务，也可以使用LocalBackend开启本地模式。它处理一般的逻辑，比如决定在job中的调度位置，唤醒推断任务。
  * 什么是speculative tasks？
 *
 * Clients should first call initialize() and start(), then submit task sets through the
 * runTasks method.
  *
  * 客户端需要首先调用initialize()和start()方法，然后通过runTasks方法提交task
 *
 ...
 * we are holding a lock on ourselves.
  *
  * SchedulerBackends和任务提交的客户端，可以以多线程的方式提交任务，所以在一些开放的api中需要加锁。
  *
  * 不理解上面的话
 */
