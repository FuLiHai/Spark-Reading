...
private[spark] class TaskSetManager(
   ...
   * it just failed.
   *
   * 有时候excutor终止了，driver就知道它不能工作了。那么一段时间内，不会给它分配任务
   */
  ...
  // TaskSet中至少有一个task运行完成或者整个taskset被抛弃；zombie状态会一直保持到所有的task都执行完成；之所以让TaskSetManager处于zombie状态，是因为这时可以跟踪所有正在运行的task。
  var isZombie = false
